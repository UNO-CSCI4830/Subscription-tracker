...
| Stakeholders: Users | | |
| When the application starts, the app shall load the user's saved subscriptions from a persistent storage. The app shall not lose data when the application or browser is closed. | | |
| Origin: Included in initial project proposal. | | |
| Version: 1.0 | Date: 02/22/2026 | Priority: 2 |
...
| Stakeholders: Users | | |
| The app shall allow users to create an account by entering an email address and password. During registration, the app shall validate that the email/password format is valid and other required fields are not empty. After account creation, the app shall display a success message and ask whether the user wants to log in. | | |
| Origin: Derived from the need to distinguish and store data for different users in the backend. Also from business aspects. | | |
| Version: 1.0 | Date: 02/22/2026 | Priority: 2 |
...
| Stakeholders: Users | | |
| The app shall allow users to login and logout using email as username and a password. The app shall validate user information, if failed, the app shall display an error message and shall not log in the user. If the user enters their password 5 times incorrectly, they shall be locked out of their account for 10 minutes. | | |
| Origin: Derived from the need for users to switch between different accounts. | | |
| Version: 1.0 | Date: 02/22/2026 | Priority: 2 |

| **FR12. Guest mode** | | |
| --- | | | --- | --- |
| Goal: The app will provide a guest mode that allows users to use the app without creating an account. | | |
| Stakeholders: Users | | |
| The app shall allow users to access core functions (adding, sorting etc.. ) in guest mode without logging in. If a feature requires an account, the app shall display a message and direct users to log in or create an account. | | |
...
| Stakeholders: Users | | |
| The app shall calculate an estimated total monthly cost based on the user's saved subscriptions. The dashboard list should display the monthly total in a visible section. | | |
| Origin: Aims to help users understand their spending and avoid waste. | | |
| Version: 1.0 | Date: 02/22/2026 | Priority: 2 |

| **FR14. Configure Email notification enrollment** | | |
| --- | | | --- | --- |
| Goal: The app will allow users to configure whether email notifications are enabled and which email address should receive them. | | |
| Stakeholders: Users | | |
| The app shall allow users to enable or disable email notifications. Also the app shall allow users to enter and update the email address used for notification delivery. | | |
| Origin: In order to improve user experience, different users may have different notification preferences, and the email to receive notification may be different from the user's registration email. | | |
| Version: 1.0 | Date: 02/22/2026 | Priority: 2 |
...
| Stakeholders: Users | | |
| The app shall support reminder timing options of 1 day, 3 days, and 7days before the renewal date. | | |
| Origin: In order to improve user experiences. | | |
| Version: 1.0 | Date: 02/22/2026 | Priority: 2 |

| **NFR2: Performance** | | |
| --- | | | --- | --- |
| Goal: The app shall provide fast responses and continue to perform acceptably under occasional heavier workloads. | | |
| Stakeholders: All | | |
| Users are able to input more than 50 subscription records, and the system should load the dashboard within 2 seconds under normal PC setup and normal network conditions. Sorting and filtering operations shall update the display list under 1 seconds. | | |
...
| Stakeholders: All | | |
| A first time user should be able to understand and use the software immediately. The interface should present clear labels for all required and un-required input fields (name, cost, cycle etc..). | | |
...
| Stakeholders: All | | |
| The app shall save subscription changes reliably so that updates persist after page refresh and browser restart. The app should send reminder emails reliably. | | |
| Origin: Added based on system reliability consideration. Reliable core features are essential for the app to be usable in everyday situations. | | |
| Version: 1.0 | Date: 02/22/2026 | Priority: 2 |
...
| Stakeholders: All | | |
| The codebase should separate UI logic, functional logic, and login logic to support future updates. The Code structure should be organized with clear documentations. | | |
...
| Stakeholders: All | | |
| The web interface shall be fully functional and visually consistent on the current version and two previous major versions of Google Chrome, Mozilla Firefox, Microsoft Edge, and Apple Safari. If a user accesses the site via an unsupported browser, the system shall display a graceful degradation message advising the user to update their browser for a secure experience. | | |
...
| Stakeholders: All | | |
| The app shall use a clean, organized, and visually appealing interface design. The interface design should improve overall user experience and not make the app harder to use or reducing system performance. | | |
| Origin: Proposed by team during discussion. | | |
| Version: 1.0 | Date: 02/28/2026 | Priority: 2 |
